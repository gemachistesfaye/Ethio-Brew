    ...
    <div className="min-h-screen bg-[#FDFCF8] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[40px] shadow-2xl p-8 md:p-12 border border-gray-50">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500">{sent ? 'Check your email for the reset code' : 'Enter your email to receive a reset code'}</p>
        </div>
        ...
        {sent ? (
          <div className="text-center">
            <div className="mb-6 p-4 bg-green-50 text-[#006341] rounded-2xl text-sm font-bold">
              If an account exists with <strong>{email}</strong>, a reset code has been sent.
            </div>
            <button
              onClick={() => navigate(`/verify-otp?email=${encodeURIComponent(email)}&purpose=reset`)}
              className="w-full bg-[#006341] text-white py-5 rounded-2xl font-bold shadow-xl hover:bg-[#004d32] transition"
            >
              Enter Reset Code
            </button>
            <p className="mt-4 text-sm text-gray-500">
              Didn't receive it? Check spam or <button onClick={() => { setSent(false); }} className="text-[#006341] font-bold">try another email</button>
            </p>
          </div>
        ) : (
